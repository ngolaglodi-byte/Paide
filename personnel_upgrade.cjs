...
// ═══ 1. DB: Add document columns ═══
const cols = db.prepare("PRAGMA table_info(hr_personnel)").all().map(c => c.name);
const toAdd = [
  ['id_card_url', 'TEXT'],
  ['diploma1_url', 'TEXT'],
  ['diploma2_url', 'TEXT'],
...
toAdd.forEach(([col, type]) => {
  if (!cols.includes(col)) {
    try { db.exec(`ALTER TABLE hr_personnel ADD COLUMN ${col} ${type}`); console.log(`Added column: ${col}`); } catch {}
  }
});

// ═══ 2. SERVER: Add file upload route + update personnel create ═══
let server = fs.readFileSync("/app/server.js", "utf8");

if (!server.includes("/api/internal/hr/personnel/upload")) {
...
`;
  ...
  console.log("Upload routes already present");
}

// ═══ 3. Rewrite Personnel.tsx with dropdown + uploads ═══
const personnelPage = `import { useState, useEffect, useMemo } from 'react';
...
const POSTS_BY_LEVEL: Record<string, string[]> = {
  ministere: ['Ministre', 'Directeur de Cabinet du Ministre', 'Secrétaire Général', 'Finance', 'Plan'],
  national: ['Coordonateur', 'Coordonateur Adjoint', 'Finance', 'Plan', 'Formation'],
  provincial: ['Coordonateur', 'Coordonateur Adjoint', 'Finance', 'Plan', 'Formation'],
  sous_provincial: ['Coordonateur', 'Coordonateur Adjoint', 'Finance', 'Plan', 'Formation'],
  centre: ['Chef de Centre', 'Chef de Centre Adjoint', 'Chargé des Opérations', 'Intendant', 'Disciplinaire'],
};
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <textarea className="w-full p-2 border rounded mt-3" rows={2} placeholder="Adresse" value={form.address||''} onChange={e => setForm({...form, address: e.target.value})} />
            </div>
...
console.log("Personnel.tsx updated with posts dropdown + uploads");
